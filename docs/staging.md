...
6. Post-deploy smoke on both `STAGING_BASE_URL` and `STAGING_OPS_URL`: `/api/health/live`, authenticated `/api/health`, login, map tiles, one admin write
...
`npm run staging:exercise` runs migrate → seed → encrypted backup smoke → destructive restore into a disposable PostgreSQL. The weekly workflow builds and starts separate public and ops Next.js staging apps (or uses both `STAGING_BASE_URL` and `STAGING_OPS_URL`) before national and authenticated load certification.
